...
export default function ResortsPage() {
  ...
  const resortsPerPage = 6;
   ...
   useEffect(() => {
    if (resorts.length > 0) {
      const itemList = resorts.map((resort, index) => ({
        '@type': 'ListItem',
        position: index + 1,
        name: resort.name,
        url: `https://hillescape.com/resorts/${resort.id}`,
        description: resort.description,
        image: resort.images?.[0] || '',
        price: `₹${resort.pricing?.basePrice || 0}`,
        priceCurrency: 'INR',
      }));
...
// Update or add the script tag
let scriptTag = document.getElementById('resorts-jsonld') as HTMLScriptElement | null;
if (!scriptTag) {
  scriptTag = document.createElement('script');
  scriptTag.id = 'resorts-jsonld';
  scriptTag.type = 'application/ld+json';
  document.head.appendChild(scriptTag);
}
scriptTag.textContent = JSON.stringify(listingSchema);
      scriptTag.textContent = JSON.stringify(listingSchema);
    }
  }, [resorts]);
